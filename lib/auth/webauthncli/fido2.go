...
func fido2Login(
	...
	deviceCallback := func(dev FIDODevice, info *deviceInfo, pin string) error {
		...
		var uName string
		if passwordless {
			cred, err := getPasswordlessCredentials(dev, info, pin, rpID, user)
			if err != nil {
				return trace.Wrap(err)
			}

			bioSkip = cred == nil // Let the device choose the credential.
			if cred != nil {
				creds = [][]byte{cred.ID}
		...
		log.Debugf(
			...
			base64.RawURLEncoding.EncodeToString(resp.CredentialID), resp.UserID, uName)
		...
		Response: &proto.MFAAuthenticateResponse_Webauthn{
			Webauthn: &wanpb.CredentialAssertionResponse{
				...
				Response: &wanpb.AuthenticatorAssertionResponse{
					...
					UserHandle:        assertionResp.UserID,
				},
...
func getPasswordlessCredentials(dev FIDODevice, info *deviceInfo, pin, rpID, user string) (*libfido2.Credential, error) {
	// Unless the user requested a specific login, assert without querying for
	// credentials. Saves a touch for bio devices.
	if info.bioEnroll && user == "" {
		log.Debugf("FIDO2: Skipping credential search for biometric device")
		return nil, nil
	}
