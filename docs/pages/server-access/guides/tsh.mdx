...
- Configure HTTP headers to be included in requests via config file
...
`tsh` has an optional configuration file that can be created in `$HOME/.tsh/config/config.yml`.
This file allows specifying HTTP headers to include when making requests 
to proxies matched on the `proxy` field.

```yaml
add_headers:
  - proxy: "*.example.com" # the proxy field supports globbing
    headers: # headers are pairs to include in the http headers
      foo: bar
```

Once this config file is in place, all HTTP requests to matching proxies will include
